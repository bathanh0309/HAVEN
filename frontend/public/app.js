...
const AUTO_RECONNECT_INTERVAL = 3000; // 3 seconds
...
const MAX_RECONNECT_ATTEMPTS = 0; // 0 = infinite retries
...
function connectToStream() {
    ...
    videoStream.onerror = (error) => {
        ...
        isConnected = false;
        showOverlay('Lỗi kết nối. Đang thử kết nối lại...');
...
function startAutoReconnect() {
    ...
    if (MAX_RECONNECT_ATTEMPTS > 0 && reconnectAttempts >= MAX_RECONNECT_ATTEMPTS) {
        ...
        showOverlay('Không thể kết nối tới backend. Vui lòng kiểm tra server.');
        return;
    }
    ...
    console.log(`Auto-reconnecting in ${delay}ms (attempt ${reconnectAttempts + 1})`);
